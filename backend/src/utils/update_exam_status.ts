...
export async function updateExamStatuses() {
  try {
    ...
    const ongoingResult = await prisma.exam.updateMany({
      where: {
        status: "SCHEDULED",
        startsAt: { lte: now },
        OR: [
    ...
    const completedResult = await prisma.exam.updateMany({
      where: {
    ...
    if (ongoingResult.count > 0 || completedResult.count > 0) {
      ...
      if (ongoingResult.count > 0) console.log(`  - Started ${ongoingResult.count} exam(s)`);
      if (completedResult.count > 0) console.log(`  - Completed ${completedResult.count} exam(s)`);
    }
  } catch (error) {
