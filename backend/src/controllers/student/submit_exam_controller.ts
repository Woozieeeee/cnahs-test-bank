...
export const submitExamController = async (req: Request, res: Response) => {
  try {
    ...
    const difficulty = exam.difficulty as ExamDifficulty;
    ...
    const hasPassedDifficulty = allAttemptsForDifficulty.some(
      (a) => a.score >= exam.passingScore && ["SUBMITTED", "AUTO_SUBMITTED", "COMPLETED"].includes(a.status)
    );
    ...
    console.log(
      `[SubmitExam] SubjectProgress updated. Student passed difficulty: ${hasPassedDifficulty}`
    );
