 ...
 */
export const getSubjectAnalyticsController = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    const averageScore =
      totalAttempts > 0
        ? Math.round(
            allExamAttempts.reduce((sum, a) => sum + a.score, 0) /
              totalAttempts
          )
        : 0;
    ...
    const questionSuccessRates = subject.questions.map((q) => {
      ...
      const correctAnswers = q.studentAnswers.filter(
        (a) => a.isCorrect
      ).length;
    ...
    const averageQuestionSuccessRate =
      questionSuccessRates.length > 0
        ? Math.round(
            questionSuccessRates.reduce((a, b) => a + b, 0) /
    ...
    const readinessDistribution = {
      ...
      atRisk: Math.ceil(
        ((100 - passingRate) / 100) * estimatedTotalStudents
      ),
    };
    ...
    const studentPerformance = {
      ...
      averagePerformers: allExamAttempts.filter(
        (a) => a.score >= 60 && a.score < 85
      ).length,
