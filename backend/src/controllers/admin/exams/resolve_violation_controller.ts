...
import { resolveViolationService } from "../../../services/admin/exams/resolve_violation_service";

export const resolveViolationController = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    const updated = await resolveViolationService(
      violationId,
      adminName,
      notes,
    );
