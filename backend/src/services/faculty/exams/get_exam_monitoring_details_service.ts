...
export async function getExamMonitoringDetailsService(examId: number, facultyId: number) {
  ...
  const studentData = exam.attempts.map((attempt) => {
    ...
    const studentRiskLevel: "LOW" | "MEDIUM" | "HIGH" = studentViolations >= 5 ? "HIGH" : studentViolations >= 2 ? "MEDIUM" : "LOW";
