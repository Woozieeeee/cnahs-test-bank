...
exports.getExamMonitoringDetailsService = getExamMonitoringDetailsService;
const prisma_1 = __importDefault(require("../../../lib/prisma"));
async function getExamMonitoringDetailsService(examId, facultyId) {
    ...
    const studentData = exam.attempts.map((attempt) => {
        const studentViolations = exam.violations.filter((v) => v.studentId === attempt.studentId).length;
        const studentRiskLevel = studentViolations >= 5 ? "HIGH" : studentViolations >= 2 ? "MEDIUM" : "LOW";
        let status = attempt.status === "IN_PROGRESS" ? "ACTIVE" :
            attempt.status === "FLAGGED" ? "FLAGGED" :
                "COMPLETED";
        return {
            ...
            submissionTime: attempt.submittedAt?.toISOString() || attempt.responses[0]?.answeredAt?.toISOString(),
    ...
    const activityFeed = exam.violations.slice(0, 50).map((v) => ({
        ...
        severity: v.severity.toLowerCase() === "high" ? "ERROR" : v.severity.toLowerCase() === "medium" ? "WARNING" : "INFO",
        isViolation: true,
        type: v.type,
    }));
    return {
