...
exports.getExamActiveStudentsService = getExamActiveStudentsService;
const prisma_1 = __importDefault(require("../../../lib/prisma"));
async function getExamActiveStudentsService(examId, facultyId) {
    ...
    const [attempts, questionCount, violations] = await Promise.all([
        prisma_1.default.examAttempt.findMany({
            where: { examId },
            include: { student: { select: { id: true, name: true, studentId: true } }, responses: { orderBy: { answeredAt: "desc" }, take: 1 } },
        }),
        prisma_1.default.examQuestion.count({ where: { examId } }),
        prisma_1.default.examViolation.groupBy({
            by: ["studentId"],
    ...
    return attempts
        ...
        const riskLevel = violationCount >= 5 ? "HIGH" : violationCount >= 2 ? "MEDIUM" : "LOW";
        const status = attempt.status === "COMPLETED" || attempt.status === "SUBMITTED" || attempt.status === "AUTO_SUBMITTED"
            ? "COMPLETED"
