...
exports.getSubjectDetails = void 0;
 ...
 */
const getSubjectDetails = async (studentId, subjectIdOrSlug) => {
    ...
    const subject = await prisma.subject.findUnique({
        where: typeof subjectIdOrSlug === 'number'
            ? { id: subjectIdOrSlug }
    ...
    const hasAccess = !!student.sectionId &&
    ...
    if (!hasAccess) {
        console.error(`[SubjectDetails Service] Student ${studentId} does not have access to subject ${subjectId}`);
        return null;
    }
    ...
    const highestScoreByExam = new Map();
    ...
    const highestScoreByDifficulty = new Map();
    fullSubject.exams.forEach((exam) => {
    ...
    difficulties.forEach((difficulty) => {
        const threshold = fullSubject.difficultyThresholds.find((t) => t.difficulty === difficulty);
    ...
    difficulties.forEach((difficulty) => {
        const threshold = fullSubject.difficultyThresholds.find((t) => t.difficulty === difficulty);
        const passingScore = threshold?.passingScore || 75;
        const examsInTier = fullSubject.exams.filter((exam) => exam.difficulty === difficulty);
        const exams = examsInTier.map((exam) => {
