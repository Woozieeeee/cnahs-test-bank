...
exports.AdvancedGradingService = exports.AggregateMetrics = exports.PredictiveMetrics = exports.TimelineMetrics = exports.ComparisonMetrics = exports.SubjectMetricsCalculator = exports.TierMetricsCalculator = exports.ExamMetrics = exports.QuestionLevelMetrics = void 0;
...
class QuestionLevelMetrics {
    /**
     * Phase 1.2: Calculate question accuracy across multiple questions
     */
    static calculateQuestionAccuracy(correctAnswers, totalQuestions) {
        if (totalQuestions === 0)
            return 0;
     ...
     */
    static calculateEfficiencyRatio(difficulty, actualTimeSpent) {
     ...
     */
    static calculateAverageTimePerQuestion(totalTimeSpent, totalQuestions) {
        if (totalQuestions === 0)
            return 0;
        return totalTimeSpent / totalQuestions;
    }
}
exports.QuestionLevelMetrics = QuestionLevelMetrics;
// ==================== PHASE 2: Single Exam Metrics ====================
class ExamMetrics {
    /**
     * Phase 2.1: Calculate raw exam score
     */
    static calculateRawScore(correctAnswers, totalQuestions) {
        if (totalQuestions === 0)
            return 0;
        return (correctAnswers / totalQuestions) * 100;
    }
     ...
     */
    static calculateWeightedScore(questionResults) {
     ...
     */
    static calculateAnswerDistribution(submitted, unanswered, flagged) {
     ...
     */
    static calculatePercentageBreakdown(correct, wrong, unanswered) {
     ...
     */
    static isPassed(score, difficulty) {
        return score >= PASSING_SCORES[difficulty];
    }
    /**
     * Phase 2.6: Calculate efficiency score for single exam
     */
    static calculateExamEfficiency(submittedCount, totalQuestions, actualTotalTime, standardTotalTime) {
     ...
     */
    static getCompleteExamMetrics(questionResults, totalTime, difficulty) {
        ...
        const standardTime = questionResults.reduce((sum, q) => sum + STANDARD_TIMES_SECONDS[q.difficulty], 0);
        const efficiency = this.calculateExamEfficiency(correctCount, totalQuestions, totalTime, standardTime);
        const answerDistribution = this.calculatePercentageBreakdown(correctCount, wrongCount, 0);
        return {
            rawScore,
            weightedScore,
            correctCount,
...
exports.ExamMetrics = ExamMetrics;
// ==================== PHASE 3: Difficulty Tier Metrics ====================
class TierMetricsCalculator {
    /**
     * Phase 3.1: Calculate comprehensive tier statistics
     */
    static calculateTierStats(scores) {
        if (scores.length === 0) {
            return {
                totalAttempts: 0,
                passedAttempts: 0,
        ...
        // Improvement trend
        const firstThreeAvg = scores.length >= 3
        ...
        // Mastery score
        const masteryScore = passRate * 0.4 + average * 0.4 + consistencyScore * 0.2;
        return {
     ...
     */
    static calculatePassRate(passedAttempts, totalAttempts) {
        if (totalAttempts === 0)
            return 0;
     ...
     */
    static calculateImprovementTrend(scores) {
        if (scores.length < 3)
            return 0;
        const firstThreeAvg = scores.slice(0, 3).reduce((a, b) => a + b, 0) /
            Math.min(3, scores.length);
        const lastThreeAvg = scores.slice(-3).reduce((a, b) => a + b, 0) / Math.min(3, scores.length);
        return lastThreeAvg - firstThreeAvg;
    }
    /**
     * Phase 3.4: Calculate consistency score
     */
    static calculateConsistencyScore(standardDeviation) {
        return Math.max(0, 100 - standardDeviation);
    }
    /**
     * Phase 3.6: Calculate tier mastery score
     */
    static calculateMasteryScore(passRate, averageScore, consistencyScore) {
        return passRate * 0.4 + averageScore * 0.4 + consistencyScore * 0.2;
    }
}
exports.TierMetricsCalculator = TierMetricsCalculator;
// ==================== PHASE 4: Subject-Level Metrics ====================
class SubjectMetricsCalculator {
    /**
     * Phase 4.1: Calculate subject overall average
     */
    static calculateSubjectAverage(easyAvg, mediumAvg, hardAvg, expertAvg, weights = {
        easy: 0.15,
        medium: 0.25,
        hard: 0.35,
        expert: 0.25,
    }) {
        return (easyAvg * weights.easy +
     ...
     */
    static calculateSubjectPassRate(easyPassed, easyTotal, mediumPassed, mediumTotal, hardPassed, hardTotal, expertPassed, expertTotal) {
     ...
     */
    static calculateCumulativeMastery(easyMastery, mediumMastery, hardMastery, expertMastery) {
        const weights = { easy: 1.0, medium: 1.3, hard: 1.6, expert: 2.0 };
        const totalWeight = weights.easy + weights.medium + weights.hard + weights.expert;
        return ((easyMastery * weights.easy +
     ...
     */
    static calculateLearningVelocity(currentMonthAverage, previousMonthAverage, days = 30) {
        return (currentMonthAverage - previousMonthAverage) / days;
    }
    /**
     * Phase 4.7: Calculate target readiness
     */
    static calculateTargetReadiness(passRate, cumulativeMastery) {
     ...
     */
    static getGPA(grade) {
        return GRADE_SCALE[grade]?.gpa || 0;
    }
}
exports.SubjectMetricsCalculator = SubjectMetricsCalculator;
// ==================== PHASE 5: Comparison and Benchmarking ====================
class ComparisonMetrics {
    /**
     * Phase 5.1: Compare current to personal best
     */
    static calculatePersonalBestComparison(currentScore, personalBest) {
        if (personalBest === 0)
            return 0;
     ...
     */
    static calculatePercentileRank(studentScore, allScores) {
     ...
     */
    static calculatePerformanceGap(studentAverage, classAverage) {
        return studentAverage - classAverage;
    }
}
exports.ComparisonMetrics = ComparisonMetrics;
// ==================== PHASE 6: Progress Timeline and Trends ====================
class TimelineMetrics {
    /**
     * Phase 6.1: Calculate weekly average
     */
    static calculateWeeklyAverage(scoresInWeek) {
     ...
     */
    static calculateAttemptsToMastery(scores, masteryThreshold = 80) {
     ...
     */
    static calculateRetentionRate(averageNow, averageNDaysAgo) {
        if (averageNDaysAgo === 0)
            return 0;
        return (averageNow / averageNDaysAgo) * 100;
    }
}
exports.TimelineMetrics = TimelineMetrics;
// ==================== PHASE 7: Predictive Metrics ====================
class PredictiveMetrics {
    /**
     * Phase 7.1: Calculate next tier readiness
     */
    static calculateNextTierReadiness(passRate, average) {
        return passRate * 0.6 + average * 0.4;
    }
    /**
     * Phase 7.2: Estimate completion time
     */
    static estimateCompletionTime(requiredPasses, currentPasses, averageTimePerExamMinutes, examsPerWeek) {
     ...
     */
    static identifyWeakAreas(topicScores, averageScore, standardDeviation) {
     ...
     */
    static identifyStrongAreas(topicScores, averageScore, standardDeviation) {
        const threshold = averageScore + 0.5 * standardDeviation;
        return Object.entries(topicScores)
            .filter(([_, score]) => score >= threshold)
            .map(([topic, _]) => topic);
...
exports.PredictiveMetrics = PredictiveMetrics;
// ==================== PHASE 8: Aggregate Metrics ====================
class AggregateMetrics {
    /**
     * Phase 8.1: Calculate overall GPA
     */
    static calculateOverallGPA(subjectGPAs) {
     ...
     */
    static calculateMultiSubjectAverage(subjectAverages, weights) {
     ...
     */
    static calculateParticipationMetrics(totalExams, weeksSinceStart, activeDays, totalDays) {
        const examsPerWeek = weeksSinceStart > 0 ? totalExams / weeksSinceStart : 0;
        const activityConsistency = totalDays > 0 ? (activeDays / totalDays) * 100 : 0;
        return { examsPerWeek, activityConsistency };
    }
    /**
     * Phase 8.4: Calculate engagement score
     */
    static calculateEngagementScore(examsPerWeek, activityConsistency, averageTimePerExamSeconds) {
...
// ==================== MAIN SERVICE ====================
class AdvancedGradingService {
    /**
     * Generate complete student performance report
     */
    static async generateStudentReport(studentId, subjectIds) {
     ...
     */
    static getRecommendation(score, passRate, trend) {
...
exports.AdvancedGradingService = AdvancedGradingService;
exports.default = AdvancedGradingService;
