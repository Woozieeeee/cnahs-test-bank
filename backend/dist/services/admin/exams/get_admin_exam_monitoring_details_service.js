...
exports.getAdminExamMonitoringDetailsService = getAdminExamMonitoringDetailsService;
const prisma_1 = __importDefault(require("../../../lib/prisma"));
async function getAdminExamMonitoringDetailsService(examId) {
    ...
    const timeRemainingMinutes = endsAt && now < endsAt
        ? Math.max(0, Math.round((endsAt.getTime() - now.getTime()) / (60 * 1000)))
        : 0;
    // Get last activity
    ...
    const studentData = exam.attempts.map((attempt) => {
        ...
        const studentRiskLevel = studentViolations >= 5
            ? "HIGH"
            : studentViolations >= 2
                ? "MEDIUM"
                : "LOW";
        ...
        return {
            ...
            submissionTime: attempt.submittedAt?.toISOString() ||
    ...
    const activityFeed = exam.violations.slice(0, 50).map((v) => ({
        ...
        severity: v.severity.toLowerCase() === "high"
            ? "ERROR"
