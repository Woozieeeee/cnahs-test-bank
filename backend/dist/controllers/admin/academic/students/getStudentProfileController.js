...
exports.getStudentProfileController = void 0;
...
const getStudentProfileController = async (req, res) => {
    try {
        ...
        // Get enrollments
        const enrollments = await prisma_1.default.section.findMany({
            where: {
                users: {
                    some: { id: studentId },
                },
        ...
        // Get exam attempts
        const examAttempts = await prisma_1.default.examAttempt.findMany({
            where: { studentId },
            include: {
                exam: {
                    select: {
        ...
        const totalAttempts = await prisma_1.default.examAttempt.count({
            where: { studentId },
        });
        const passedAttempts = await prisma_1.default.examAttempt.count({
            where: {
                studentId,
                passed: true,
            },
        });
        const averageScore = await prisma_1.default.examAttempt.aggregate({
            where: { studentId },
            _avg: { score: true },
        });
