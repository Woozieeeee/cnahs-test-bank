...
function AdminExamMonitoringCard({ exam }: { exam: Exam }) {
  ...
  const getTimerColor = () => {
    if (timerLabel.includes("Exam Ended"))
      return "text-muted-foreground";
    if (timerLabel.includes("Time Remaining"))
      return "text-orange-600";
    if (timerLabel.includes("Starts In"))
  ...
  return (
    <MotionCard>
      <Link
        href={`/admin/exams/monitoring/${exam.id}`}
        className="block"
      >
          ...
          <div className="flex h-full flex-col">
            ...
            {timeRemaining && (
              <div className="mt-5">
                ...
                <p
                  className={`mt-1 text-2xl font-bold ${getTimerColor()}`}
                >
                  {timeRemaining}
                </p>
              </div>
...
export default memo(AdminExamMonitoringCard);
