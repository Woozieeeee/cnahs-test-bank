...
import { useExamDetails } from "@/hooks/faculty/exams/useExamDetails";
import { useExamMonitoringActions } from "@/hooks/faculty/exams/useExamMonitoringActions";
...
function ExamDetailPage({ params }: ExamDetailPageProps) {
  const resolvedParams = use(params);
  const examId = parseInt(resolvedParams.examId);

  const { loading, error, data: exam, refresh } = useExamDetails(examId, { 
    pollInterval: 5000, 
    autoRefresh: true 
  });
  ...
  } = useExamMonitoringActions({ examId, onSuccess: refresh });
